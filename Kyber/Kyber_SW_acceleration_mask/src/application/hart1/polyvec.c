...
__attribute__((optimize("O0"))) void polyvec_hw_ntt(polyvec *r, int rd)
{
    for(int i=0;i<KYBER_K;i++) {
        ...
        MSS_GPIO_set_output(GPIO2_LO, MSS_GPIO_9, 1u);
        while (sign_end == 0) {}
        sign_end = 0;
        MSS_GPIO_set_output(GPIO2_LO, MSS_GPIO_9, 0u);
    }
...
__attribute__((optimize("O0"))) void polyvec_mult(poly *r, polyvec *a, polyvec *b)
{
    for(int i = 0; i < KYBER_K; i++) {
        write_coeffs_1(&b->vec[i]);
